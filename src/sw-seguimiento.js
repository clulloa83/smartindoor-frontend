importScripts('./sw-utilidades.js');
...
const syncSeguimientos = async() => {

    try {

        const endpoint = '/api/seguimiento';
        const usuario_key = 'usuario';
        const key = 'seguimientos';
        
        const usuario = await obtenerStore(usuario_key);

        const seguimientos = await obtenerStore(key);

        const Nosincronizados = seguimientos.map(m => {
            if(!m.sincronizado){
                return { ...m };
            }
        });
        ...
        if(Nosincronizados.length > 0){
            const promesas = [];
            Nosincronizados.forEach((seguimiento) => {
                promesas.push(registrarSeguimiento(seguimiento, usuario.token, endpoint))
            });
    
            const sincronizados = await Promise.all(promesas);
            await guardarSeguimientoSync(sincronizados, key);
        };        
